...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv2 = nn.Conv2d(in_channels=self.n_encode, out_channels=2 * self.n_encode, kernel_size=5, stride=2,
                               padding=2)
        self.conv3 = nn.Conv2d(in_channels=2 * self.n_encode, out_channels=4 * self.n_encode, kernel_size=5, stride=2,
                               padding=2)
        self.conv4 = nn.Conv2d(in_channels=4 * self.n_encode, out_channels=8 * self.n_encode, kernel_size=5, stride=2,
                               padding=2)
...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        self.conv4t = nn.ConvTranspose2d(in_channels=2 * self.n_generator, out_channels=self.n_generator,
                                         kernel_size=4, stride=2, padding=1)
        self.conv5t = nn.ConvTranspose2d(in_channels=self.n_generator, out_channels=3, kernel_size=3, stride=1,
                                         padding=1)
        self.relu = nn.ReLU(inplace=False)
        self.tanh = nn.Tanh()
...
class Dimg(nn.Module):
    ...
    def __init__(
        ...
        self.conv1t = nn.ConvTranspose2d(self.labels * self.n_age + self.n_gender,
                                         self.labels * self.n_age + self.n_gender,
